...
- [x] **`onDidReceiveDebugSessionCustomEvent` SSE event** — adapter-defined custom DAP events. NOTE: this does NOT carry standard DAP events (`stopped`, `continued`, …) — VSCode's API intercepts those before they reach extensions. The autonomous-debugging loop is closed by `onDebugAdapterEvent` instead.
- [x] **`onDebugAdapterEvent` SSE event** — eagerly-registered `DebugAdapterTrackerFactory` forwards every standard DAP `event` message (`stopped`, `continued`, `terminated`, `output`, `breakpoint`, `thread`, `module`, `loadedSource`, …) to the bus. Closes the autonomous-debugging loop: step → wait for `stopped` → inspect → step. Measured ~1.2s end-to-end for set BP → hit → inspect → stepIn → stepOver → evaluate → continue → exit on a tiny node program.
